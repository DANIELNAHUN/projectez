  ...
  <div class="space-y-6 sm:space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div class="flex-shrink-0 flex gap-3">
        <button @click="showAIGenerator = true"
    ...
    <ResponsiveModal v-model:visible="showCreateForm" :title="editingProject ? 'Editar Proyecto' : 'Nuevo Proyecto'"
      size="lg" :content-padding="false" @close="closeForm">
      <ProjectForm :project="editingProject" @submit="handleProjectSubmit" @cancel="closeForm" />
    </ResponsiveModal>

    <ResponsiveModal v-model:visible="showEditForm" :title="editingProject ? 'Editar Proyecto' : 'Nuevo Proyecto'"
      size="lg" :content-padding="false" @close="closeForm">
      <ProjectForm :project="editingProject" @submit="handleProjectSubmit" @cancel="closeForm" />
    </ResponsiveModal>

    <!-- Delete Confirmation Modal -->
    <ResponsiveModal v-model:visible="showDeleteConfirm" title="Confirmar Eliminación" size="sm"
      :close-on-backdrop="false" @close="showDeleteConfirm = false">
      <div class="space-y-4">
    ...
    <AIProjectGenerator v-model:visible="showAIGenerator" @project-imported="handleAIProjectImported" />

    <!-- Project Import/Export Component -->
    <ProjectImportExport @project-imported="handleProjectImported" @project-exported="handleProjectExported" />

    <!-- User Management Demo (temporary for testing) -->
    <UserManagementDemo v-if="showUserManagementDemo" />

    <!-- Projects List -->
    <ProjectList @create-project="showCreateForm = true" @edit-project="handleEditProject"
      @delete-project="handleDeleteProject" @view-project="handleViewProject" @retry="loadProjects" />
  </div>
</template>
