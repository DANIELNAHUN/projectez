...
const actions = {
  ...
  async loadCalendarEvents({ commit, rootGetters }) {
    ...
    try {
      ...
      const validTasks = allTasks.filter(task => {
        const hasValidStartDate = task.startDate && !isNaN(new Date(task.startDate).getTime())
        const hasValidEndDate = task.endDate && !isNaN(new Date(task.endDate).getTime())
  ...
  async loadCalendarEventsAndNavigate({ commit, dispatch, rootGetters }) {
    // Load calendar events first
    await dispatch('loadCalendarEvents')

    // Navigate to earliest task date if there's a current project
    const currentProject = rootGetters['projects/currentProject']
    if (currentProject) {
      await dispatch('navigateToEarliestTask')
    }
  },
  ...
  async navigateToEarliestTask({ commit, state, rootGetters }) {
    try {
      ...
      // Navigate to the month of the earliest task
      commit('SET_SELECTED_DATE', earliestDate)
    } catch (error) {
      console.error('Error navigating to earliest task:', error)
    }
  },
  ...
  async onProjectChange({ dispatch }, projectId) {
    // This action should be called when the project changes
    // It will reload events and navigate to the earliest task
    if (projectId) {
      await dispatch('loadCalendarEventsAndNavigate')
    } else {
      // If no project selected, just reload events
      await dispatch('loadCalendarEvents')
    }
  },
...
const getters = {
  ...
  // Get earliest task date for current project
  getEarliestTaskDate: (state, getters, rootState, rootGetters) => {
    const currentProject = rootGetters['projects/currentProject']
